...
def ocr_without_segment(filepath):
	...
	# ocr text
	bocr = pytesseract.image_to_string(Image.open(ocr_directory + "/winelabelbinary.png"))
	bocrStrip = bocr.strip()
	bocrStripFilter = ''.join(e for e in bocrStrip if e.isalnum())
	# if bocrStripFilter != "":
	ocr_recognized_text.append(bocrStripFilter)
	# add text region
	text_region_image_path.append(ocr_directory + "/winelabelbinary.png")
...
def main():
	if len(sys.argv)>1:
		...
		for filename in ordered_files:
			...
			text_region_image_path, ocr_recognized_text = ocr_without_segment(filepath)
