...
def main():
	if len(sys.argv)>2:
		...
		global orb_kp_dist,sift_kp_dist,surf_kp_dist
		...
		for filename in ordered_files:
			...
			method=myfunc.path_extension(filename)
			# print wineid + " , " + method
			if method == ".orb":
				orb_kp_des = pickle.load( open(desfilepath, "rb") )
				orb_kp, orb_des = myfunc.unpickle_keypoints(orb_kp_des)
				orb_kp_dist[wineid] = orb_kp
				orb_des_dist[wineid] = orb_des
			elif method == ".sift":
				sift_kp_des = pickle.load( open(desfilepath, "rb") )
				sift_kp, sift_des = myfunc.unpickle_keypoints(sift_kp_des)
				sift_kp_dist[wineid] = sift_kp
				sift_des_dist[wineid] = sift_des
			elif method == ".surf":
				surf_kp_des = pickle.load( open(desfilepath, "rb") )
				surf_kp, surf_des = myfunc.unpickle_keypoints(surf_kp_des)
				surf_kp_dist[wineid] = surf_kp
				surf_des_dist[wineid] = surf_des
...
def ir(testfilepath):
	...
	for wineid, des in sift_des_dist.iteritems():
		...
		print "len(matches):"+ str(len(matches)) + " len(good):"+str(len(good))

	print "sift_match_wineid: " + sift_match_wineid
